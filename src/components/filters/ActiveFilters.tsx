...
import { formatFilterValue as formatNumberValue } from '@/utils/filterUtils';
import { FilterValue } from './filter-types';

interface FilterTipItem {
  filterId: string;
  text: string;
  onClear: () => void;
}
      ...
      if (isArrayFilter && Array.isArray(filter.value)) {
        formattedValues.forEach((text, index) => {
          ...
          tips.push({
            filterId: filter.id,
            text,
            onClear: () => clearFilterItem(filter.id, itemValue),
          });
        });
        ...
        tips.push({
          ...
          onClear: () => updateFilter(filter.id, undefined),
        });
      } else {
        ...
        formattedValues.forEach((text, index) => {
          ...
          if (key) {
            tips.push({
              filterId: filter.id,
              text,
              onClear: () => clearFilterField(filter.id, key),
            });
          }
        });
      }
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {filterTips.map((tip, index) => (
        <FilterTip key={`${tip.filterId}-${index}`} text={tip.text} onClear={tip.onClear} />
      ))}
    </div>
  );
};
