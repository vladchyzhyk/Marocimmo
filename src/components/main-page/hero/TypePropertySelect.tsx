...
export default function TypePropertySelect({
  ...
  // Sync tempSelection with value when value changes externally
  useEffect(() => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        setIsOpen(false);
        // Reset temp selection if closed without applying
        setTempSelection(value);
      }
    };
  ...
  const handleToggle = () => {
    if (isOpen) {
      // Close and reset temp selection
      setIsOpen(false);
      setTempSelection(value);
    } else {
      // Open and sync temp selection
      setTempSelection(value);
      setIsOpen(true);
    }
  };
            ...
            {options.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`body-lg flex-1 transition-colors ${
                      isChecked
