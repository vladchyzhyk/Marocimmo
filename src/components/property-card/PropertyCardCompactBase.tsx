...
import type { PropertyCardBaseProps } from './types';

export const PropertyCardCompactBase = ({
  title,
  price,
  currency = 'DH',
  propertyType,
  ...
  return (
    <div onClick={() => onCardClick?.(id || '')}>
      <div
        className={`relative w-full border border-[var(--border)] rounded-[8px] overflow-hidden bg-[var(--white)] hover:shadow-lg transition-shadow ${className}`}
      >
        {showUndo && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] z-20 flex items-center justify-center rounded-[8px]">
            <button
              type="button"
              onClick={onUndoClick}
              className="px-6 py-2 border border-[var(--border)] rounded-lg text-[var(--color-black)] body-md hover:bg-[var(--bg-tint)] transition-colors bg-white shadow-sm"
              aria-label="Undo"
            >
              Undo
            </button>
          ...
          <div className="flex-1 p-2 pb-4 flex flex-col justify-between relative">
            <div className="flex flex-col items-start gap-2 relative">
              <div className="flex flex-col items-start gap-2 w-full">
                ...
                <div className="flex items-center gap-2">{propertyIcons}</div>
              </div>
              {showContacts ? (
